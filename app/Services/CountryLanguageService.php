...
class CountryLanguageService
{
    ...
    public function getBothByCode($country, $language)
    {
        return $this->model->where('code', $country)
            ->first()
            ->languages()
            ->where('code', $language)
            ->first();
    ...
    public function getTopRankLanguageByCode($country, $others = [])
    {
        return $this->model->where('code', $country)
            ->first()
            ->public_active_languages()
            ->orderBy('is_primary', 'DESC')
            ->orderByRaw("FIELD(status_public, 'enabled', 'draft')")
            ->orderBy('code')
            ->first();
        }
