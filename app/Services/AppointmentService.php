...
class AppointmentService
{
    ...
    public function list(Request $request)
	{
        $orderable = [
            ...
            'places.area',
            [
                'c_members.first_name',
                'c_members.middle_name',
                'c_members.last_name'
            ],
            'crew_members_count',
            'scheduled_at',
     ...
     * @return Collection<Appointment>
     */
    public function getWithPublicServices($company = null, $country = null, $order = null, $others = [])
    {
        return $this->model
            ...
            }, 'place.city', 'place.district', 'services'])
            ->filterBy('status', request()->query('status'))
            ->orderBy('serviced_at', 'DESC')
            ->directOrder($order)
            ->when(isset($others['limit']), function($query) use ($others) {
                return $query->limit($others['limit']);
            })
            ->get();
    }
