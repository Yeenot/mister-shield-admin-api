    ...
    <div class="kt-portlet">
        ...
        <div class="kt-portlet__body">
            ...
            <div class="form-group">
                ...
                <br>
                {{ !empty($appointment->contract) ? $appointment->contract->code .' '. optional($appointment->contract->started_at)->format('d/m/Y') .' - '. optional($appointment->contract->ended_at)->format('d/m/Y') : '' }} 
            </div>

            <div class="form-group">
                <label>{{ __('staff/forms.appointments_ordered_at') }}</label>
                <br>
                {{ !empty($appointment->ordered_at) ? $appointment->ordered_at->setTimeZone(app('shared')->get('others')['timezone'])->format('d/m/Y H:i:s') : '' }} 
            </div>

            <div class="form-group">
                <label>{{ __('staff/forms.appointments_scheduled_at') }}</label>
                <br>
                {{ !empty($appointment->scheduled_at) ? $appointment->scheduled_at->setTimeZone(app('shared')->get('others')['timezone'])->format('d/m/Y H:i:s') : '' }} 
            </div>

            <div class="form-group">
                <label>{{ __('staff/forms.appointments_serviced_at') }}</label>
                <br>
                {{ !empty($appointment->serviced_at) ? $appointment->serviced_at->setTimeZone(app('shared')->get('others')['timezone'])->format('d/m/Y H:i:s') : '' }} 
            </div>
            ...
            @if ($appointment->payment_term == 'postpaid')
                <div class="form-group">
                    ...
                    <br>
                    {{ optional($appointment->payment_due_at)->format('d/m/Y H:i:s') }} 
                </div>
            @endif
